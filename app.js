...
// Telegram Mini App init + robust button binding for iOS Telegram
(function(){
  ...
  window.state = window.state || { user: null, balances: {real:0, virtual:100}, match:null };
  state.user = tgUser || { login: 'guest' };
  if (!state.balances) state.balances = {real:0, virtual:100};
  if (typeof renderBalances === 'function') renderBalances();

  function initButtons(){
    var startBtn = document.getElementById('startMatchBtn');
    var addBtn   = document.getElementById('addVirtualBtn');
    var buyBtn   = document.getElementById('buyRealBtn');
    var backBtn  = document.getElementById('backToLobby');

    if(startBtn && !startBtn.dataset.bound){
      startBtn.dataset.bound = true;
      startBtn.addEventListener('click', function(){
        var type = document.getElementById('matchType').value;
        var stake = Math.max(1, parseInt(document.getElementById('stake').value || "0", 10));
        if (type === "virtual_bot") {
          if (state.balances.virtual < stake) return alert("Недостаточно виртуальных звёзд");
          state.balances.virtual -= stake;
          startGame({ type, stake, rake: 0, opponent: "bot" });
        } else if (type === "real_online") {
          var rake = Math.floor(stake * 0.10);
          var total = stake + rake;
          if (state.balances.real < total) return alert("Нужно " + total + " реальных звёзд (включая 10% рейк)");
          state.balances.real -= total;
          startGame({ type, stake, rake, opponent: "stub_online" });
        }
